...
def init_args():
    ...
    # basic settings
    parser.add_argument("--pretrain_model_path", default='/root/data3/bert-base-uncased', type=str,
...
class T5FineTuner(pl.LightningModule):
    ...
    def forward(self, input_ids, attention_mask=None, decoder_input_ids=None,
        ...
        # print("decoder_input_ids:", decoder_input_ids.device)
        # print("decoder_attention_mask:", decoder_attention_mask.device)
        ...
        # prefixs = outputs.encoder_last_hidden_state[:, 0, :]
        # # print("prefixs:", prefixs)
        # # print(decoder_input_ids)
        # suffixs = self.model.encoder(input_ids=decoder_input_ids, attention_mask=decoder_attention_mask)
        # # print("suffixs", suffixs)
        # suffixs = suffixs.last_hidden_state[:, 0, :]
        # # print("suffixs:", suffixs)
        # rankgen_loss = self.rank_loss(prefixs, suffixs)
        #
        # # self.total_rank_loss += rankgen_loss.item()
        # # self.total_t5_loss += outputs.loss.item()
        # # self.total_loss += (rankgen_loss.item() + outputs.loss.item())
        # # print(f"mean rank loss: {self.total_rank_loss / self.total_steps}\t "
        # #       f"mean t5 loss: {self.total_t5_loss / self.total_steps}\t "
        # #       f"mean total loss: {self.total_loss / self.total_steps}")
        # outputs.loss += rankgen_loss

        return outputs

    def rank_loss(self, logits, labels):
        # zp = torch.sum(torch.exp(torch.matmul(logits, labels)), dim=-1)
        # print("logits:", logits)
        # print("labels:", labels)
        # zp = torch.exp(torch.matmul(logits, labels.t()))
        # print("zp:", zp.shape)
        zpi = torch.sum(torch.exp(torch.matmul(logits, labels.t()).squeeze(dim=0)), dim=-1)
        # print("zpi:", zpi)
        # print("zpi:", zpi.shape)
        # print("logits_i:", logits[0].view(-1).shape)
        # print("labels_i:", labels[0].view(-1).shape)
        # debug = torch.exp(torch.matmul(logits[0].view(1, -1), labels[0].t().view(-1, 1)).squeeze(dim=0))
        # print("__debug__", debug.shape)
        # p_ci_pi = torch.exp(torch.matmul(logits, labels.t()).squeeze(dim=0)) / zpi
        p_ci_pi = torch.cat(
            [torch.exp(torch.matmul(logits[i].view(1, -1), labels[i].view(-1, 1)).squeeze(dim=0)) for i in
             range(logits.shape[0])],
            dim=0) / zpi
        # p_ci_pi = torch.cat(
        #     [torch.exp(torch.cosine_similarity(logits[i].view(1, -1), labels[i].view(1, -1))) for i in range(logits.shape[0])],
        #     dim=0) / zpi
        # print("p_ci_pi:", p_ci_pi.shape)
    ...
    def _step(self, batch):
        ...
        # rankgen_loss = self.rankgen_train(batch)
        # print("rankgen_loss", rankgen_loss)
        # print("loss", loss)
...
def evaluate(data_loader, model, dataset):
    """
    ...
    scorer_model.load_state_dict(torch.load(f'../step1_ori/outputs/{args.dataset}/model.pth'))
    scorer_model.cuda()
    scorer_model.eval()
    ...
    for batch in tqdm(data_loader):
        curr_outs = model.model.generate(input_ids=batch['source_ids'].to(device),
        ...
        a = scorer_model(**a)
        tmp_scores = []
        for t in curr_outs_text:
            print(t)
            t = scorer_tokenizer(t, return_tensors='pt')
            t = scorer_model(**t)
            tmp_scores.append(torch.matmul(a, t.t()).squeeze(dim=0).item()/768)
    ...
    # out_data_file = f'./res_generation/{args.dataset}/{args.seed}.txt'
    # with open(out_data_file, 'w', encoding='utf-8') as f:
    #     for i in range(len(targets)):
    #         f.write(f'{texts[i][0]}\n')
    #         f.write(f'{targets[i]}\n')
    #         for s in all_outputs[i]:
    #             f.write(f'{s}\n')
    #         f.write('\n')
    ...
    scores, all_labels, all_preds = compute_scores(outputs, targets, dataset)
...
if args.do_direct_eval:
    ...
    scores = evaluate(test_loader, model, args.dataset)
