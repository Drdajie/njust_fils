...
class T5FineTuner1(pl.LightningModule):
    """
    Fine tune a pre-trained T5 model
    """

    def __init__(self, args, tfm_model, tokenizer, vocab_size=32130):
        super(T5FineTuner1, self).__init__()
        self.args = args
        self.model = tfm_model
...
def evaluate(data_loader, model, model1, args):
    """
    ...
    scorer_model.load_state_dict(torch.load(f'../step1/outputs/{args.dataset}/model.pth'))
    scorer_model.cuda()
    scorer_model.eval()

    for batch in tqdm(data_loader):
        pre1 = batch['source_ids']       # 输入的前缀
        # print(batch['text'])
        curr_outs1 = model1.model.generate(input_ids=pre1.to(device),
                                        attention_mask=batch['source_mask'].to(device),
                                        max_length=128,
                                        return_dict_in_generate=True,
                                        num_return_sequences=8,
                                        num_beams=8)  # num_beams=8, early_stopping=True)
        curr_outs_text1 = model1.tokenizer.batch_decode(curr_outs1['sequences'], skip_special_tokens=True)
        text1 = batch['text'][0]
        a1 = scorer_tokenizer(text1, return_tensors='pt')
        a1 = scorer_model(**a1)
        tmp_scores1 = []
        for b1 in curr_outs_text1:
            b1 = scorer_tokenizer(b1, return_tensors='pt')
            b1 = scorer_model(**b1)
            score1 = (torch.matmul(a1, b1.t()).squeeze(dim=0).item()/768)
            tmp_scores1.append(score1)
        tmp_scores1 = np.array(tmp_scores1)
        tmp_scores1 = softmax(tmp_scores1)
        tmp_scores1 = tmp_scores1.tolist()
        index1 = tmp_scores1.index(max(tmp_scores1))

        if tmp_scores1[index1] - tmp_scores1[0] < 0.7:
            index1 = 0

        # index = args.index
        dec1 = [curr_outs_text1[index1]]           # 0~7

        all_outputs.append(curr_outs_text1)
        target1 = [tokenizer.decode(ids, skip_special_tokens=True) for ids in batch["target_ids"]]
        ...
        # print(batch['text'])
        pre_text = batch['text'][0]
        pre_set = []                    # ['Holder refers to A.', 'Holder refers to B.']
        for i in range(4):#args.hops):
            ...
            a = scorer_tokenizer(pre_text, max_length=128, return_tensors='pt')
            a = scorer_model(**a)
            tmp_scores = []
            for b in curr_outs_text:
                b = scorer_tokenizer(b, max_length=128, return_tensors='pt')
                b = scorer_model(**b)
            ...
            if tmp_scores[index] - tmp_scores[0] < 0.2:
                index = 0
            ...
            pre_text = batch['text'][0] + ' ' + ' [SSEP] '.join(pre_set)
            pre = tokenizer(pre_text, return_tensors="pt").input_ids
    ...
    scores, all_labels, all_preds = compute_scores(outputs, targets, args.dataset, args.seed, args.index)
...
if args.do_train:
    ...
    tfm_model1 = T5ForConditionalGeneration.from_pretrained(args.model_name_or_path)
    ...
    tfm_model1.resize_token_embeddings(len(tokenizer))
    print("tfm_model.config.vocab_size: ", tfm_model.config.vocab_size)
    model = T5FineTuner(args, tfm_model, tokenizer)
    model1 = T5FineTuner1(args, tfm_model1, tokenizer)
    ...
    torch.save(model.state_dict(), f"./T5_Args/{args.dataset}/ssat5_{args.num_train_epochs}.pkl")
    model1.load_state_dict(torch.load(f"../t5_recurrent/T5_Args/{args.dataset}/ssat5.pt"))
else:
    print("\n****** Conduct Training ******")
    train_start = time.time()
    # initialize the T5 model
    ...
    model = T5FineTuner(args, tfm_model, tokenizer)
    model1 = T5FineTuner1(args, tfm_model1, tokenizer)
    model.load_state_dict(torch.load(f"./T5_Args/{args.dataset}/ssat5_{args.num_train_epochs}.pkl"))
    model1.load_state_dict(torch.load(f"../t5_recurrent/T5_Args/{args.dataset}/ssat5.pt"))
...
if args.do_direct_eval:
    ...
    print('*'*100)
    scores = evaluate(test_loader, model, model1, args)
